 ...
 * @since 0.1.0
 */
fun getCurrentDateTimeInUTC(format: String = defaultFormat): String =
    ZonedDateTime.now(utcZone).format(DateTimeFormatter.ofPattern(format.ifBlank { defaultFormat }))
 ...
 * @since 0.1.0
 */
fun getCurrentDateTimeInUTCAsISO8601(): String =
    ZonedDateTime.now(utcZone).format(DateTimeFormatter.ISO_OFFSET_DATE_TIME)
 ...
 * @since 0.1.0
 */
fun Instant.toHumanReadableInUTC(format: String = defaultFormat): String =
    DateTimeFormatter.ofPattern(format.ifBlank { defaultFormat }).format(this.atZone(utcZone))
...
fun Long.toHumanReadableTime(): String = listOf(
    ...
    .joinToString(", ") {
        "${
            NumberFormat.getNumberInstance(Locale.ENGLISH).format(it.first)
        } ${it.second}${if (it.first > 1) "s" else ""}"
    }
